...
public sealed class DeviceRepository(IDbConnectionFactory connectionFactory)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sql =
            ...
            from core.devices
            where (@OwnerUserId is null or owner_user_id = @OwnerUserId)
    ...
    public async Task<DeviceDto?> GetByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<DeviceDto>(new CommandDefinition(
            $"select {DeviceColumns} from core.devices where id = @Id::char(36)",
            new { Id = id.ToString() },
            cancellationToken: cancellationToken));
    }

    public async Task<DeviceDto> CreateAsync(CreateDeviceRequest request, CancellationToken cancellationToken)
    {
        using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);
    ...
    public async Task<DeviceDto?> UpdateAsync(Guid id, UpdateDeviceRequest request, CancellationToken cancellationToken)
    {
        using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);
        ...
        return await connection.QuerySingleOrDefaultAsync<DeviceDto>(new CommandDefinition(
            sql,
            new { Id = id.ToString(), request.Name, request.IsActive, request.AppId },
            cancellationToken: cancellationToken));
    }
    ...
    public async Task<bool> DeleteAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        var affected = await connection.ExecuteAsync(new CommandDefinition(
            "delete from core.devices where id = @Id::char(36)",
            new { Id = id.ToString() },
            cancellationToken: cancellationToken));
    ...
    public async Task<bool> OwnerExistsAsync(Guid ownerUserId, CancellationToken cancellationToken)
    {
        using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);

        return await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            "select exists(select 1 from auth.users where user_id = @OwnerUserId)",
